...
This directory contains the Flatpak manifest (`com.ccswitch.desktop`) for CC Switch-KP, used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
```bash
cp "$(find src-tauri/target/release/bundle -name '*.deb' | head -n 1)" flatpak/cc-switch-kp.deb
```
...
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo CC-Switch-KP-Linux.flatpak com.ccswitch.desktop
```

4) Install and run:

```bash
flatpak install --user ./CC-Switch-KP-Linux.flatpak
