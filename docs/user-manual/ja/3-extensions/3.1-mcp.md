...
> **注意**：OpenClaw と Claude Desktop は現在 CC Switch-KP MCP 同期に対応していません。MCP 機能は Claude、Codex、Gemini、OpenCode、Hermes に対応しています。

### スイッチの動作

あるアプリのスイッチをオンにすると、CC Switch-KP は以下を実行します：
...
あるアプリのスイッチをオフにすると、CC Switch-KP は以下を実行します：
...
CLI ツールで既に MCP サーバーを設定している場合、CC Switch-KP にインポートできます：

1. 「インポート」ボタンをクリック
2. インポートするアプリを選択（Claude/Codex/Gemini/OpenCode/Hermes）
3. CC Switch-KP が既存の設定を読み取ってインポート
