...
CC Switch-KP automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with CC Switch-KP configuration
[Expand] [Dismiss]
```
...
3. CC Switch-KP will automatically back up and remove the selected variables
...
If you prefer not to use CC Switch-KP to remove variables, you can handle them manually:
...
- CC Switch-KP provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use CC Switch-KP to manage configurations**: Avoid setting API keys in system environment variables
