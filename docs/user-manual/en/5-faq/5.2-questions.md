...
CC Switch-KP for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/farion1231/cc-switch/releases).
...
2. Add CC Switch-KP to your antivirus software's whitelist
...
chmod +x CC-Switch-KP-*.AppImage

# If it still fails, try
./CC-Switch-KP-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Confirm the file was exported by CC Switch-KP
...
**Windows**:
- Check taskbar settings to ensure the CC Switch-KP icon is not hidden
...
Toggle "Lightweight Mode" from the system tray menu. The main window closes, and CC Switch-KP runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, CC Switch-KP covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `ccswitch://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
4. Provide the following information:
   ...
   - CC Switch-KP version
   - Problem description and reproduction steps
   - Error messages (if any)

### Log Files

CC Switch-KP stores logs under its application configuration directory. By default this is `.cc-switch` in your home directory; if you changed the configuration directory in Advanced Settings, use that custom location instead.
