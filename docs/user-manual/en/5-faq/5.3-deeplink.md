...
CC Switch-KP supports the `ccswitch://` deep link protocol, enabling one-click configuration import via links.
...
CC Switch-KP provides an online deep link generator tool:
...
Using CC Switch-KP's official online deep link generator tool is more convenient.
...
1. The system asks whether to open CC Switch-KP
2. After confirming, CC Switch-KP opens
...
CC Switch-KP automatically registers the `ccswitch://` protocol during installation.
...
```bash
/usr/bin/open -a "CC Switch-KP" --args --register-protocol
```
...
Before import, CC Switch-KP will:
