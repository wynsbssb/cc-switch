...
Hermes uses YAML configuration. CC Switch-KP writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to CC Switch-KP's "providers") |
...
CC Switch-KP's priority when modifying configurations:

1. **CC Switch-KP Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by CC Switch-KP)
- CC Switch-KP's `settings.json`
...
CC Switch-KP v3.7.0 migrated from JSON files to SQLite:
