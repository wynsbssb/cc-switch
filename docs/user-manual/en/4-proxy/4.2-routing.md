...
App routing means letting CC Switch-KP route a specific application's API requests through the local routing service.
...
When routing is enabled, CC Switch-KP modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, CC Switch-KP will:
...
Before enabling routing, CC Switch-KP backs up the original configuration and restores it when disabled.
