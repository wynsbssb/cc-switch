...
- 从下方 [Assets](#assets) 下载 `CC-Switch-KP-v3.6.1-macOS.zip`

> **注意**：由于作者没有苹果开发者账号，首次打开可能出现"未知开发者"警告。请前往"系统设置" → "隐私与安全性" → 点击"仍要打开"

#### Windows

- **安装包**：`CC-Switch-KP-v3.6.1-Windows.msi`
- **便携版**：`CC-Switch-KP-v3.6.1-Windows-Portable.zip`
...
- **自定义存储位置** - 自定义 CC Switch-KP 的配置存储目录
...
CC Switch-KP 是一个跨平台桌面应用，用于管理和切换 Claude Code 与 Codex 的不同供应商配置。基于 Tauri 2.0 + React 18 + TypeScript 构建，支持 Windows、macOS、Linux。
