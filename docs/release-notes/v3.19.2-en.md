...
OMO 4.19.3 unified its configuration into `~/.omo/omo.jsonc` (then `omo.json`), and its migration renames the legacy per-app file out of the way. From then on CC Switch-KP — which only knew the legacy path — kept writing a file outside OMO's config chain, so provider switches looked successful while landing nowhere ([#5945](https://github.com/farion1231/cc-switch/issues/5945)).
...
The identity file Hermes loads from `~/.hermes/` is `SOUL.md`; it never looks for `AGENTS.md` there — that name is project-level context, discovered upward from the working directory. CC Switch-KP's prompt management had written `~/.hermes/AGENTS.md` since the day Hermes support was added, so enabling a Hermes prompt produced a dead file. It now reads and writes `~/.hermes/SOUL.md`, and the existing back-fill still applies: a SOUL.md you wrote yourself is imported into the database before being replaced. ([#5777](https://github.com/farion1231/cc-switch/issues/5777))
...
| File                                     | Description                                      |
| ---------------------------------------- | ------------------------------------------------ |
| `CC-Switch-KP-v3.19.2-Windows.msi`          | **Recommended** - MSI installer with auto-update |
| `CC-Switch-KP-v3.19.2-Windows-Portable.zip` | Portable build, unzip and run                    |
...
| File                             | Description                                           |
| -------------------------------- | ----------------------------------------------------- |
| `CC-Switch-KP-v3.19.2-macOS.dmg`    | **Recommended** - DMG installer, drag to Applications |
| `CC-Switch-KP-v3.19.2-macOS.zip`    | Unzip and drag to Applications, Universal Binary      |
| `CC-Switch-KP-v3.19.2-macOS.tar.gz` | For Homebrew install and auto-update                  |
...
- `CC-Switch-KP-v3.19.2-Linux-x86_64.AppImage` / `.deb` / `.rpm`
- `CC-Switch-KP-v3.19.2-Linux-arm64.AppImage` / `.deb` / `.rpm`
