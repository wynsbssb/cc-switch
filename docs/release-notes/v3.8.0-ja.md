...
CC Switch-KP v3.8.0 はデータ永続化レイヤーと UI を大幅に作り替え、今後のクラウド同期やローカルプロキシ機能に向けた基盤を整えたメジャーアップデートです。
...
- **Windows**: `CC-Switch-KP-v3.8.0-Windows.msi` または `-Portable.zip`
- **macOS**: `CC-Switch-KP-v3.8.0-macOS.tar.gz` または `.zip`
- **Linux**: `CC-Switch-KP-v3.8.0-Linux.AppImage` または `.deb`
