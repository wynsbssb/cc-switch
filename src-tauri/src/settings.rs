...
pub struct AppSettings {
    ...
    /// Automatically snapshot the Codex Desktop conversation data
    /// (~/.codex sessions / state DB) so switching to the ChatGPT official
...
pub fn backup_codex_desktop_conversations() -> bool {
    settings_store()
        .read()
        .unwrap_or_else(|e| {
            log::warn!("设置锁已毒化，使用恢复值: {e}");
