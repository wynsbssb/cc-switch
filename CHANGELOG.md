...
All notable changes to CC Switch-KP will be documented in this file.
...
- **Codex Model Aggregation Can Auto-Add a Provider's Discoverable Models**: In the official Codex provider's aggregation section, pick a target provider and click "自动添加该供应商全部模型" to fetch its `/v1/models` (or managed xAI/Codex OAuth account) model list and append one custom model row per discovered model — each row is pre-bound to that provider with display id, upstream model and display name defaulting to the fetched model id, and already-added rows are skipped. The per-row upstream model field also gains a fetch button that populates its dropdown (merged with any saved provider catalog) and auto-fills the first model when empty, so providers without a saved model catalog no longer require hand-typing every model id.
...
- **切换 Codex 供应商/路由不再从零重建 live 配置（统一应用配置）**: 此前每次切换路由都会用新供应商的存储配置整体重建 `~/.codex/config.toml`，之前 live 里的应用级共享配置（`sqlite_home`、`[tui]`、`[approval_policy]`、`[sandbox]`、用户手写的 `web_search` 等）会丢，需要重新配置。现在所有 Codex live 落盘路径（普通切换、本地路由热切换、启动同步、接管恢复）都会把当前 live 文件中的应用级共享配置合并进新生成配置（表结构深合并，新配置已有的键优先），供应商专属键（`model_provider` / `base_url` / `model_providers` / bearer token / 模型目录 / MCP / 插件）仍以新供应商为准：插件继续走原有自动保留，MCP 仍由数据库重投影。切换路由不再从 0 开始配置。
...
- **Codex Model Aggregation No Longer Loses Configured Models in the Desktop Picker**: With model aggregation enabled, the Desktop picker only shows models that pass Statsig's `available_models` whitelist gate. cc-switch injected the aggregate/custom model ids into the cached whitelist, but the freshness "pin" was only applied to evaluation bundles isolated to the models config — the real Desktop cache bundles the models config with hundreds of unrelated gates/configs, so the injected ids were overwritten on the next SDK refresh, and cc-switch cannot re-inject while the Desktop holds the LevelDB lock. Every evaluation entry that carries the models config is now pinned once all configured ids are present — shared bundles use a bounded 12-hour horizon (instead of freezing unrelated evaluations for weeks) that the background worker renews, so configured models stay visible instead of silently disappearing.
- **Switching Models Mid-Conversation No Longer 400s on Stale Reasoning/Tool Items**: Replaying a conversation that started on another provider could send `reasoning` items carrying plaintext `content` arrays (or legacy `function_call`/`function_call_output` items with a `content` field) to a strict native Responses upstream, which rejects them with `Invalid 'input[x].content': array too long. Expected an array with maximum length 0`. The proxy now normalizes those `content` arrays to `[]` on every native Responses passthrough (Codex and Grok Build alike, before the request leaves the proxy), and additionally retries once after the rejection signature if an upstream is still hit — so the fix covers every provider regardless of how the gateway words its error. Vendors that require `reasoning_text` round-trips (e.g. DeepSeek) are detected and deliberately keep their reasoning arrays intact. (#36551, #36704)
...
- **Hermes Prompts Were Written to a File Hermes Never Reads**: Hermes loads its persona file `SOUL.md` from `~/.hermes/` and never looks for `AGENTS.md` there — that name is project-level context discovered from the working directory upward. CC Switch-KP's prompt management wrote `~/.hermes/AGENTS.md` from the day Hermes support was added, so enabling a Hermes prompt produced a dead file. Prompts now read and write `~/.hermes/SOUL.md`, and the existing back-fill still applies: a SOUL.md you wrote yourself is imported into the database before being replaced. (#5777)
...
- **Grok Build Joins the Failover Tabs and the Environment-Conflict Check**: The failover settings gain a fourth tab for Grok Build alongside Claude Code, Codex and Gemini, and the startup environment-conflict banner now detects a shell-exported `XAI_API_KEY` or `GROK_DEFAULT_MODEL` — variables that silently override whatever provider is selected. Detection distinguishes exact names from prefixes, so CC Switch-KP's own `GROK_BIN_DIR` and `GROK_HOME` are not reported.
...
- The mirrored DeepSeek catalog declares a minimum Codex client version of 0.144.0, which CC Switch-KP does not verify — the freeform `apply_patch` registration it carries needs that release or newer. The generated catalog file also grows to roughly 75 KB for the two mirrored models, since each carries the full harness text.
...
- **Grok Official Provider With Official-Login Import**: A "Grok Official" entry lets Grok Build fall back to the CLI's own built-in xAI OAuth login: selecting it hides the connection fields and writes an empty `~/.grok/config.toml`, and CC Switch-KP never stores or touches those credentials. Live-config reads, backups, and official-state writes now use syntax-only TOML validation, so an official-login (empty) config round-trips instead of failing; importing the live config while Grok is in official login state yields "Grok Official set as current" instead of an error, matching Codex behavior. Official-state detection is deliberately wired only into the manual import command — the startup auto-importer still rejects official-state configs, so a deleted Grok Official entry is never resurrected on launch. Proxy takeover of an official-login Grok config is skipped automatically and rejected with a clear message on the manual path, consistent with the existing ban on proxying official providers.
...
- **Codex Chat Truncated Stream Detection**: When a Chat Completions upstream ends a stream without a `finish_reason` or `[DONE]`, CC Switch-KP no longer reports it as a normal completion — it finalizes normally only when the stream truly finished, emits an incomplete (`max_output_tokens`) response when partial output was produced, and emits a failed `stream_truncated` event when nothing was produced. Late-arriving reasoning is also attached to still-active streamed tool calls.
...
- **Windows Tray Icon Residue on Exit**: Quitting CC Switch-KP on Windows could leave a dead tray icon until hovered; the app now removes the tray icon before exiting so it disappears cleanly (#3797).
- **Windows Taskbar Icon**: Set an explicit Windows AppUserModelID at runtime and stamped the installer's desktop and start-menu shortcuts with the same ID and product icon, so CC Switch-KP shows the correct icon and groups properly in the taskbar (#3457).
...
- **Codex Chat Completions routing**: Codex providers can now be served by OpenAI-compatible Chat Completions upstreams. CC Switch-KP converts Codex Responses requests into Chat Completions, rebuilds JSON and SSE responses back into Responses shape, preserves reasoning / `<think>` / tool-call state, normalizes error envelopes, and probes Chat-format providers correctly in Stream Check.
...
- **Codex Chat Completions Routing**: Codex providers can now be served by upstreams that only speak the OpenAI Chat Completions API. CC Switch-KP's local proxy converts Codex's outgoing Responses requests into Chat Completions and rebuilds the Chat response (both JSON and SSE) back into Responses shape, preserving `reasoning_content`, inline `<think>` blocks, streamed reasoning summaries, tool calls, and `previous_response_id` follow-ups. A bounded Codex Chat history cache restores tool calls before their tool outputs.
...
- **Homebrew Official Cask Installation**: Installation simplified to `brew install --cask cc-switch` now that CC Switch-KP is in the official Homebrew repository; the personal-tap requirement was removed from all READMEs.
...
- **Claude Desktop Third-Party Provider Switching via Proxy Gateway**: Added a dedicated Claude Desktop surface that brokers third-party Claude providers through CC Switch-KP's in-app proxy, with a routing-support badge for providers that need it, role-based model route mapping locked to `sonnet` / `opus` / `haiku`, Copilot/Codex OAuth provider reuse, a redesigned Claude Code import flow, app-switcher differentiation between "Claude Code" and "Claude Desktop", and 44 provider presets translated from the Claude Code catalog.
...
- **Claude Desktop Operational Notes**: Switching a Claude Desktop provider now writes CC Switch-KP's managed 3P profile and requires restarting Claude Desktop to take effect; proxy-mode providers require CC Switch-KP Local Routing to stay running.
...
- **CC Switch-KP Brand Surface Unified to ccswitch.io**: All in-app and README references now point at ccswitch.io as the sole official website; the release notes template also surfaces ccswitch.io.
...
- **Hermes Config Health Scanner**: Removed the in-app Hermes config health scanner, warning banner, `scan_hermes_config_health` command, `HermesHealthWarning` type, and `HermesWriteOutcome.warnings` payload. CC Switch-KP now keeps the Hermes surface focused on active provider display, provider switching defaults, memory editing, and launching the Hermes Web UI for deep configuration.
...
- **Codex OAuth Responses SSE Aggregation**: Non-streaming Anthropic clients now receive JSON even when the ChatGPT Codex upstream forces OpenAI Responses SSE; CC Switch-KP aggregates the upstream SSE events before running the non-streaming transform (#2235).
...
- **Hermes Memory Panel**: Added a Memory panel for editing `MEMORY.md` and `USER.md` directly from CC Switch-KP, with an enable switch, character-count limits, and a live save flow. Replaces the Prompts entry for Hermes.
...
- **Application-Level Window Controls**: Added an opt-in setting to render CC Switch-KP's own minimize / toggle-maximize / close buttons instead of the system decorations, materially improving the experience on Linux Wayland where compositor-drawn buttons can become inert.
...
- **OpenClaw Config Directory Override**: Added a settings option to point CC Switch-KP at a custom `openclaw.json` location.
- **Hermes Config Directory Override**: Added a settings option to point CC Switch-KP at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch.
...
- **Hermes Deep Config Delegation**: Deep YAML knobs are now delegated to the Hermes Web UI via a direct launch action, rather than duplicated in the CC Switch-KP form.
...
- **Hermes YAML Pollution & OAuth MCP Auth Drop**: Round-tripping through CC Switch-KP no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys; guard tests added via `tests/hermes_roundtrip.rs`.
...
- **Lightweight Mode**: Added a tray-only mode that destroys the main window and keeps CC Switch-KP running from the system tray, with the window recreated when users reopen it.
...
- **Skills Discovery & Batch Updates**: Added SHA-256 based skill update detection, per-skill and batch update actions, a storage-location toggle between CC Switch-KP and `~/.agents/skills`, and public `skills.sh` search integration.
...
- **OpenClaw Integration**: Full management support for OpenClaw as the fifth application in CC Switch-KP, including provider switching, configuration panels (Env / Tools / Agents Defaults), Workspace file management (HEARTBEAT / BOOTSTRAP / BOOT), daily memory files, and additive overlay mode
...
- **Windows Title** - Display "CC Switch-KP" instead of default "Tauri app" in window title
...
- **Import/Export** - Restrict SQL import to CC Switch-KP exported backups only; refresh providers immediately after import
...
- **Standardized release artifact naming** - All platform releases now use consistent version-tagged filenames:
  - macOS: `CC-Switch-KP-v{version}-macOS.tar.gz` / `.zip`
  - Windows: `CC-Switch-KP-v{version}-Windows.msi` / `-Portable.zip`
  - Linux: `CC-Switch-KP-v{version}-Linux.AppImage` / `.deb`
