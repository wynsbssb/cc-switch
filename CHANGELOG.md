...
- **Codex Desktop Model Picker Keeps Custom / Aggregate Models with Official Login**: The Desktop picker only shows models that pass Statsig's `available_models` gate. cc-switch injected the aggregate model ids into that whitelist, but the freshness "pin" was only applied to evaluation bundles isolated to the models config — the real Desktop cache bundles hundreds of configs/gates, so the injected ids were overwritten on the next SDK refresh, and cc-switch cannot re-inject while the Desktop holds the LevelDB lock. The pin is now applied to every evaluation cache entry that carries the models config once all configured ids are present, so after a Codex Desktop restart the custom / other-provider models stay visible instead of being hidden again.
